...
{    
	...
	GPIOA->CRL&=0XFFF0000F;//PA1 anolog输入 
	...
	ADC1->CR2&=~(1<<11);   //右对齐	 
	ADC1->SQR1&=~(0XF<<20);
	ADC1->SQR1|=3<<20;     //3个转换在规则序列中 //也就是只转换规则序列1 			   
	//设置通道1的采+++样时间
	ADC1->SMPR2&=~(3*1);   //通道1采样时间清空	  
 	...
 	ADC1->SMPR2|=7<<(3*2); //通道2  239.5周期,提高采样时间可以提高精确度
        ADC1->SMPR2&=~(3*3);   //通道2采样时间清空	  
 	ADC1->SMPR2|=7<<(3*3); //通道2  239.5周期,提高采样时间可以提高精确度
        ADC1->SMPR2&=~(3*4);   //通道2采样时间清空	  
